...
CREATE TABLE REPORT (
    ...
    UPDATED_AT TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT fk_report_submitter FOREIGN KEY (SUBMITTED_BY) REFERENCES ACCOUNT(ACCOUNT_ID) ON DELETE CASCADE
);
...
CREATE TABLE ANNOUNCEMENT (
    ...
    CONSTRAINT fk_announcement_creator FOREIGN KEY (CREATED_BY) REFERENCES ACCOUNT(ACCOUNT_ID) ON DELETE CASCADE
);
...
CREATE TABLE GENERATED_REPORT (
    ...
    CONSTRAINT fk_generated_report_account FOREIGN KEY (GENERATED_BY) REFERENCES ACCOUNT(ACCOUNT_ID) ON DELETE CASCADE
);
...
CREATE TABLE RESOLUTION (
    ...
    CONSTRAINT fk_resolution_account FOREIGN KEY (RESOLVED_BY) REFERENCES ACCOUNT(ACCOUNT_ID) ON DELETE CASCADE
);
...
-- Trigger to automatically create STUDENT or STAFF record after ACCOUNT creation
CREATE OR REPLACE TRIGGER trg_account_ai
AFTER INSERT ON ACCOUNT
FOR EACH ROW
BEGIN
    IF :NEW.ACCOUNT_TYPE = 'STUDENT' THEN
        -- Don't insert yet - will be updated later with actual STUDENT_ID
        INSERT INTO STUDENT (ACCOUNT_ID, CREATED_AT, UPDATED_AT)
        VALUES (:NEW.ACCOUNT_ID, SYSTIMESTAMP, SYSTIMESTAMP);
    ELSIF :NEW.ACCOUNT_TYPE = 'STAFF' THEN
        -- Don't insert yet - will be updated later with actual STAFF_ID
        INSERT INTO STAFF (ACCOUNT_ID, ROLE, CREATED_AT, UPDATED_AT)
        VALUES (:NEW.ACCOUNT_ID, 'STAFF', SYSTIMESTAMP, SYSTIMESTAMP);
    END IF;
END;
/
...
BEGIN
    IF :NEW.EMERGENCY_ID IS NULL THEN
        SELECT emergency_seq.NEXTVAL INTO :NEW.EMERGENCY_ID FROM dual;
    END IF;
    IF :NEW.CREATED_AT IS NULL THEN
        :NEW.CREATED_AT := SYSTIMESTAMP;
    END IF;
...
BEGIN
    IF :NEW.GENERATE_ID IS NULL THEN
        SELECT generate_seq.NEXTVAL INTO :NEW.GENERATE_ID FROM dual;
    END IF;
