...
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE report_assignment_seq'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END; /
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE emergency_info_seq'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END; /
BEGIN EXECUTE IMMEDIATE 'DROP SEQUENCE generated_report_seq'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -2289 THEN RAISE; END IF; END; /
...
CREATE SEQUENCE report_assignment_seq START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE emergency_info_seq START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE generated_report_seq START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE resolution_seq START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;
...
CREATE TABLE STAFF (
    ACCOUNT_ID NUMBER PRIMARY KEY,
    ROLE VARCHAR2(20) DEFAULT 'STAFF' CHECK (ROLE IN ('STAFF', 'ADMIN', 'SUPERADMIN')),
    DEPARTMENT VARCHAR2(100),
    POSITION VARCHAR2(100),
    SUPERVISOR_ID NUMBER,
    AVATAR_URL VARCHAR2(255),
    CONSTRAINT fk_staff_account FOREIGN KEY (ACCOUNT_ID) REFERENCES ACCOUNT(ACCOUNT_ID) ON DELETE CASCADE,
    CONSTRAINT fk_staff_supervisor FOREIGN KEY (SUPERVISOR_ID) REFERENCES STAFF(ACCOUNT_ID)
);
...
CREATE TABLE REPORT_ASSIGNMENT (
    ...
    CONSTRAINT fk_assignment_staff FOREIGN KEY (ACCOUNT_ID) REFERENCES STAFF(ACCOUNT_ID) ON DELETE CASCADE,
...
CREATE TABLE ANNOUNCEMENT (
    ...
    CONSTRAINT fk_announcement_creator FOREIGN KEY (CREATED_BY) REFERENCES STAFF(ACCOUNT_ID)
);
...
CREATE TABLE GENERATED_REPORT (
    ...
    REPORT_DATA_TYPE VARCHAR2(20) NOT NULL CHECK (REPORT_DATA_TYPE IN ('SUMMARY', 'DETAILED')),
    REPORT_DATA CLOB,
    REQUESTED_AT TIMESTAMP DEFAULT SYSTIMESTAMP,
    CONSTRAINT ck_generated_report_dates CHECK (DATE_RANGE_END >= DATE_RANGE_START),
    CONSTRAINT fk_generated_report_staff FOREIGN KEY (GENERATED_BY) REFERENCES STAFF(ACCOUNT_ID)
);
...
CREATE TABLE RESOLUTION (
    ...
    CONSTRAINT fk_resolution_staff FOREIGN KEY (RESOLVED_BY) REFERENCES STAFF(ACCOUNT_ID)
);
...
CREATE INDEX idx_assignment_staff ON REPORT_ASSIGNMENT(ACCOUNT_ID);
...
-- ============================================================================
-- Set timestamps and sequence defaults on insert/update
